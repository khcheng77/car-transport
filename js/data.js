...
const DB = {
  ...
  dailyDutyMin: 13.5 * 60,  // 13.5 小時（2.13 本版修正：12.5→13.5，業務單位新數字）：涵蓋待勤/前後緩衝/裝卸/站內移動/收貨等待/行駛/休息用餐
  ...
  /* ---- 2.18 站內建物間移動時間：(拜訪棟數 − 1) × 每棟增量，獨立於裝卸時間另計入 2.13 額度 ---- */
  intraSiteMovePerBuildingMin: 30,  // 每多拜訪一棟建物之固定增量（分）
  /* ---- 2.19 收貨時間窗：車輛抵達收貨建物須落在「希望收貨時間 ～ ＋窗寬」內才能收貨 ----
     Q4 待業務確認是否全公司統一固定值；現以 4 小時（240 分）實作為可調參數。 */
  receiveWindowMin: 4 * 60, // 收貨時間窗寬（分，Q4 已確認 4 小時）
  ...
  /* ---- 3.1 各據點出差「最短天數表」：依 車型（大車/小車）× 目的地 查表 ----
     ★ 停靠與否不影響天數；此表為寬鬆估計值，僅供排班參考顯示，
       不參與運算、不反過來限制 13.5 小時精算結果。★示意值待業務提供（Q1）。 */
  minTripDays: {
